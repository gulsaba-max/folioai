               ...
               <div className="space-y-4">
                {manualProjects.map((proj, idx) => (
                 <div key={idx} className="p-4 border border-border-subtle rounded-xl relative group">
                  ...
                  <h4 className="font-medium text-sm mb-1">{proj.title || "Untitled Project"}</h4>
                  <p className="text-xs text-text-muted line-clamp-1">{proj.description}</p>
                 </div>
                ))}
            ...
            <div className="space-y-4">
             ...
             <div className="flex gap-4">
              {(['dark', 'light'] as const).map(mode => (
               <button key={mode} type="button" onClick={() => setGatherMode(mode)} className={`flex-1 py-3 rounded-2xl text-sm font-medium capitalize transition-all border ${
                gatherMode === mode ? 'border-primary text-primary bg-primary/5' : 'border-border-subtle text-text-muted bg-white hover:bg-gray-50'
               }`}>
                {mode === 'dark' ? 'ðŸŒ™ Dark' : 'â˜€ï¸ Light'}
               </button>
              ))}
             </div>
            </div>
