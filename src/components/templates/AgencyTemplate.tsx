...
export default function AgencyTemplate({
  ...
  const bg = isDark ? "#0c0c0c" : "#fafaf9";
  const text = isDark ? "#fafaf9" : "#0c0c0c";
  const muted = isDark ? "#525252" : "#a3a3a3";
  const border = isDark ? "#1c1c1c" : "#e5e5e5";

  const [hoveredProject, setHoveredProject] = useState<number | null>(null);
  const [mousePos, setMousePos] = useState({ x: 0, y: 0 });

  return (
    <div className="min-h-screen" style={{ background: bg, color: text, fontFamily: "'Cormorant Garamond', 'Inter', serif" }}>
      <link rel="stylesheet" href={GOOGLE_FONTS} />
      <style>{`
        .project-hover { transition: all 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94); }
        .project-hover:hover { background: ${isDark ? "#161616" : "#f5f5f4"}; }
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 py-6"
        style={{ background: isDark ? "rgba(12,12,12,0.9)" : "rgba(250,250,249,0.9)", backdropFilter: "blur(20px)" }}>
        <span className="text-sm font-bold tracking-wider uppercase" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <section className="min-h-screen flex flex-col justify-between pt-28 px-8 pb-12 relative overflow-hidden">
        ...
        <div className="flex-1 flex flex-col justify-center max-w-5xl">
          <motion.div initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.77, 0, 0.18, 1] }}>
            <p className="text-sm font-sans font-semibold mb-6 flex items-center gap-3" style={{ color: accent, fontFamily: "'Inter', sans-serif" }}>
              <span className="w-8 h-px" style={{ background: accent }} /> {portfolio.title}
            </p>
            <h1 className="font-black italic leading-[0.88] tracking-tight mb-8" style={{ fontSize: "clamp(4rem, 11vw, 12rem)" }}>
              {portfolio.name?.split(" ").map((w, i) => (
                <span key={i} className={i % 2 === 0 ? "" : "not-italic"}>
                  {w}{" "}
                </span>
              ))}
            </h1>
            <p className="text-lg leading-relaxed max-w-lg" style={{ color: muted, fontFamily: "'Inter', sans-serif" }}>{portfolio.bio}</p>
          </motion.div>
        </div>

        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}
          className="flex justify-between items-end">
          <div className="flex gap-4 font-sans text-xs" style={{ fontFamily: "'Inter', sans-serif", color: muted }}>
      ...
      {sectionOrder.includes("projects") && (
        <section id="portfolio-projects" className="border-t relative" style={{ borderColor: border }}>
          <div className="px-8 py-4 flex justify-between items-center border-b" style={{ borderColor: border }}>
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] font-sans" style={{ color: muted, fontFamily: "'Inter', sans-serif" }}>
              Selected Work
            </p>
            <p className="text-[10px] font-sans" style={{ color: muted, fontFamily: "'Inter', sans-serif" }}>
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}
                  ...
                  <div className="flex-1">
                    <h3 className="text-2xl md:text-3xl font-black italic mb-1 group-hover:text-current transition-colors" style={{ color: hoveredProject === i ? pal.accent : text }}>
                      {p.title}
                    </h3>
                    <p className="text-sm leading-relaxed font-sans" style={{ color: muted, fontFamily: "'Inter', sans-serif" }}>{p.description}</p>
                  </div>
      ...
      {sectionOrder.includes("skills") && (
        <section id="portfolio-skills" className="px-8 py-16 border-b" style={{ borderColor: border }}>
          <p className="text-[10px] font-bold uppercase tracking-[0.3em] mb-8 font-sans" style={{ color: muted, fontFamily: "'Inter', sans-serif" }}>
            {labels.skills}
          </p>
      ...
      {sectionOrder.includes("experience") && (
        <section id="portfolio-experience" className="px-8 py-16 border-b" style={{ borderColor: border }}>
          <p className="text-[10px] font-bold uppercase tracking-[0.3em] mb-8 font-sans" style={{ color: muted, fontFamily: "'Inter', sans-serif" }}>
            {labels.experience}
          </p>
          <div className="space-y-0">
            {portfolio.experience?.map((exp, i) => (
              <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}
                ...
                style={{ borderColor: border }}>
                <div className="col-span-3 text-xs font-mono" style={{ color: muted, fontFamily: "'JetBrains Mono', monospace" }}>
                  {exp.startDate}<br />{exp.endDate}
                </div>
                <div className="col-span-9">
                  <h3 className="text-2xl font-black italic mb-1">{exp.role}</h3>
                  <p className="text-sm font-bold mb-3 font-sans" style={{ color: accent, fontFamily: "'Inter', sans-serif" }}>{exp.company}</p>
                  <p className="text-sm leading-relaxed font-sans" style={{ color: muted, fontFamily: "'Inter', sans-serif" }}>{exp.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {sectionOrder.includes("contact") && (
        <section id="portfolio-contact" className="px-8 py-24 min-h-[70vh] flex flex-col justify-center"
          style={{ background: isDark ? "#0a0a0a" : "#f0f0ef" }}>
          <div className="max-w-3xl mx-auto w-full">
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] mb-4 font-sans" style={{ color: muted, fontFamily: "'Inter', sans-serif" }}>
              Get in Touch
            </p>
            <h2 className="font-black italic leading-none mb-12" style={{ fontSize: "clamp(3rem, 9vw, 8rem)" }}>
              Let's<br /><span style={{ color: accent }}>Collaborate.</span>
            </h2>
            {formSuccess && (
              <div className="mb-6 p-4 rounded-2xl flex items-center gap-2 text-sm font-sans"
                style={{ background: `${accent}15`, color: accent, border: `1px solid ${accent}30`, fontFamily: "'Inter', sans-serif" }}>
      ...
      <footer className="border-t py-8 px-8 flex justify-between items-center text-[11px] font-sans" style={{ borderColor: border, color: muted, fontFamily: "'Inter', sans-serif" }}>
