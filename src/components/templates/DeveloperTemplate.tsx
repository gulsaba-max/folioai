...
function TerminalHero({ name, title, email, accent }: { name: string; title: string; email: string; accent: string }) {
  const lines = [
    `$ whoami`,
    `→ ${name}`,
    `$ cat about.txt`,
  ...
  return (
    <div className="rounded-2xl overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.04)] border" style={{ borderColor: "#30363d", background: "#0d1117" }}>
      {/* Window chrome */}
      <div className="flex items-center gap-2 px-4 py-3 border-b" style={{ borderColor: "#30363d", background: "#161b22" }}>
        <div className="w-3 h-3 rounded-full bg-red-500 opacity-80" />
        <div className="w-3 h-3 rounded-full bg-yellow-500 opacity-80" />
        <div className="w-3 h-3 rounded-full bg-green-500 opacity-80" />
        <span className="ml-3 text-[11px] font-mono text-slate-500">~/portfolio — zsh</span>
      </div>
        ...
        {lines.map((line, i) => (
          <div key={i} className="mb-1 leading-relaxed">
            {displayed[i] !== undefined ? (
              <span style={{
                color: line.startsWith("$") ? "#7ee787" : line.startsWith("→") ? "#c9d1d9" : "#c9d1d9",
              }}>
                {displayed[i]}
                {i === displayed.length - 1 && <span className="animate-pulse text-white ml-0.5">▌</span>}
              </span>
            ) : null}
          </div>
        ))}
...
function SkillBar({ skill, i, accent, ...rest }: { skill: string; i: number; accent: string } & React.HTMLAttributes<HTMLDivElement>) {
...
export default function DeveloperTemplate({
  ...
  return (
    <div className="min-h-screen" style={{ background: bg, color: text, fontFamily: "'JetBrains Mono', monospace" }}>
      <link rel="stylesheet" href={GOOGLE_FONTS} />

      {/* Nav */}
      <nav className="sticky top-0 z-50 flex items-center justify-between px-6 md:px-10 py-4 border-b"
        style={{ background: "rgba(13,17,23,0.9)", backdropFilter: "blur(20px)", borderColor: border }}>
        <div className="flex items-center gap-2">
          <div className="w-5 h-5 rounded flex items-center justify-center" style={{ background: accent }}>
            <Code2 className="w-3 h-3 text-white" />
          </div>
          <span className="text-sm font-bold" style={{ color: text }}>{portfolio.name?.split(" ")[0].toLowerCase()}<span style={{ color: accent }}>.dev</span></span>
        </div>
        <div className="hidden md:flex items-center gap-6">
          {["projects", "experience", "skills"].filter(s => sectionOrder.includes(s)).map(s => (
            <a key={s} href={`#portfolio-${s}`} className="text-xs hover:opacity-70 transition-opacity font-mono" style={{ color: muted }}>/{s}</a>
          ))}
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-10 py-16 space-y-20">
        ...
        <section id="portfolio-hero">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
            ...
            <div className="flex items-center gap-2 mb-4 text-xs font-mono" style={{ color: accent }}>
              ...
              <span style={{ color: muted }}>latest commit: "Portfolio v3.0 — Available for hire"</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-black mb-3" style={{ color: "#ffffff", fontFamily: "'Inter', sans-serif" }}>
              {portfolio.name}
            </h1>
            <p className="text-lg mb-2 font-sans" style={{ color: accent }}>{portfolio.title}</p>
            <div className="flex items-center gap-4 text-sm mb-6" style={{ color: muted }}>
            ...
            <p className="text-sm leading-relaxed max-w-xl font-sans mb-8" style={{ color: muted }}>{portfolio.bio}</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            <TerminalHero name={portfolio.name} title={portfolio.title} email={portfolio.contactEmail} accent={accent} />
          </motion.div>
        </section>
        ...
        {sectionOrder.includes("projects") && (
          <section id="portfolio-projects">
            <DevSectionHeader icon={<Zap className="w-4 h-4" />} title={labels.projects} accent={accent} muted={muted} border={border} />
                ...
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-bold text-white font-sans text-sm">{p.title}</h3>
                    </div>
                    {p.link && <a href={p.link} target="_blank" rel="noreferrer"
                      className="opacity-0 group-hover:opacity-100 transition-opacity"
                      style={{ color: accent }}><ExternalLink className="w-3.5 h-3.5" /></a>}
                  </div>
                  <p className="text-xs leading-relaxed mb-4 font-sans" style={{ color: muted }}>{p.description}</p>
                  <div className="flex flex-wrap items-center gap-3">
                    <div className="flex items-center gap-1 text-[10px]" style={{ color: muted }}>
                     ...
                     {(Array.isArray(p.techStack) ? p.techStack : []).slice(1, 4).map((t, ti) => (
                      <span key={ti} className="text-[10px] px-2 py-0.5 rounded-full border" style={{ borderColor: border, color: muted }}>{t}</span>
                    ))}
                  </div>
                </motion.div>
              ))}
        ...
        {sectionOrder.includes("skills") && (
          <section id="portfolio-skills">
            <DevSectionHeader icon={<Code2 className="w-4 h-4" />} title={labels.skills} accent={accent} muted={muted} border={border} />
            <div className="grid md:grid-cols-2 gap-x-12">
              {portfolio.skills?.map((s, i) => <SkillBar key={i} skill={s} i={i} accent={accent} />)}
            </div>
          </section>
        )}
        ...
        {sectionOrder.includes("experience") && (
          <section id="portfolio-experience">
            <DevSectionHeader icon={<Briefcase className="w-4 h-4" />} title={labels.experience} accent={accent} muted={muted} border={border} />
              ...
              {portfolio.experience?.map((exp, i) => (
                <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}
                  ...
                  <div className="absolute -left-[9px] top-7 w-4 h-4 rounded-full border-2 flex items-center justify-center"
                    style={{ borderColor: i === 0 ? accent : border, background: bg }}>
                    <div className="w-1.5 h-1.5 rounded-full" style={{ background: i === 0 ? accent : border }} />
                  </div>
                  <div className="flex justify-between flex-wrap gap-2 mb-1.5">
                    <div>
                      <h3 className="font-bold text-white font-sans">{exp.role}</h3>
                      <p className="text-sm font-semibold" style={{ color: accent }}>{exp.company}</p>
                    </div>
                    <span className="text-[10px] font-mono px-2 py-1 rounded" style={{ background: card, color: muted }}>
                      {exp.startDate} → {exp.endDate}
                    </span>
                  </div>
                  <p className="text-sm leading-relaxed font-sans" style={{ color: muted }}>{exp.description}</p>
                </motion.div>
              ))}
            </div>
          </section>
        ...
        {sectionOrder.includes("education") && (
          <section id="portfolio-education">
            <DevSectionHeader icon={<GraduationCap className="w-4 h-4" />} title={labels.education} accent={accent} muted={muted} border={border} />
            <div className="grid md:grid-cols-2 gap-4">
              {portfolio.education?.map((edu, i) => (
                <div key={i} className="rounded-xl p-5 border" style={{ background: card, borderColor: border }}>
                  <h3 className="font-bold text-white font-sans mb-0.5">{edu.degree}</h3>
                  <p className="text-sm font-sans" style={{ color: muted }}>{edu.school}</p>
        ...
        {sectionOrder.includes("contact") && (
          <section id="portfolio-contact">
            <DevSectionHeader icon={<Mail className="w-4 h-4" />} title="Contact" accent={accent} muted={muted} border={border} />
            {formSuccess && (
            ...
            <form onSubmit={handleSendMessage} className="space-y-3 max-w-xl">
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} type="submit" disabled={formLoading}
                className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold cursor-pointer font-sans shadow-[0_8px_30px_rgba(0,0,0,0.04)]"
      ...
      <footer className="border-t py-8 text-center text-[11px] font-mono" style={{ borderColor: border, color: muted }}>
