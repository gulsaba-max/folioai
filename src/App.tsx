...
export default function App() {
  ...
  // When true, always show the onboarding (upload/manual) page regardless of portfolio state
  // [isOnboarding state already declared above]
  ...
  if (!currentUser) {
    return (
      <Layout currentUser={null} onGetStarted={() => setShowAuth(true)} onLogout={handleLogout} onOpenPanel={setActivePanel}>
        ...
        {!showAuth ? (
          <LandingPage onGetStarted={() => setShowAuth(true)} onOpenPanel={setActivePanel} />
        ) : (
  ...
  return (
    <Layout currentUser={currentUser} onGetStarted={() => setShowAuth(true)} onLogout={handleLogout} onOpenPanel={setActivePanel}>
      ...
      <div className="flex h-[calc(100vh-72px)] w-full overflow-hidden relative bg-bg-base">
